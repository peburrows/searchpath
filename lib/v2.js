(function(){
  ...
  SearchpathV2.prototype.close = function() {
    document.getElementById('searchpath_wrapper');
  };

  SearchpathV2.prototype.render = function(html) {
    if( !document.getElementById('searchpath_wrapper') ) {
      var create = document.createElement
        , wrapper= create('div')
        , els    = ['arrow', 'pane', 'backdrop'];

      wrapper.setAttribute('id', 'searchpath_wrapper');
      document.body.appendChild(wrapper);
      for(var i=0; i<els.length; i++) {
        var div = create('div');
        div.setAttribute('id', 'searchpath_'+els[i]);
